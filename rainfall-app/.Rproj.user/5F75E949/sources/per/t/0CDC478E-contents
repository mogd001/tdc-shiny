...
default_start_date <- ymd("2022-07-12")
...
get_rainfall_data <- function(from = format(default_start_date, "%Y%m%d"), to = "") {
  get_data(collection = "Rainfall", method = "Total", time_interval = NA, from = from, to = "", interval = "15 minutes", alignment = "00:00") %>%
...
server <- function(input, output, session) {
  ...
  rainfall_data <- reactive({
    ...
    # Filter to date range to calcite total
    x <- rainfall() %>% 
